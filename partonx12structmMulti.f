...
c     This program makes use of a few cuda routines
      Subroutine SetCT18Multi(Tablefile)    
      Implicit Double Precision (A-H,O-Z)
      Character Tablefile*40, tableN*40
     ...
     > / CtqPar1 / qBase,XV(0:MXX),TV(0:MXQ),UPD(MXPQX), AlsCTEQ(0:mxq)
      ...
      common / ctmulti / updmulti(MXPQXTOT), icallset
      save
      ...
      do iset = 0, 58
         ...
         if (iset .lt. 10) then
            tableN(j+1:j+1) = '0'
            tableN(j+2:j+2) = char(int(iset)+48)
         else
            tableN(j+1:j+1) = char(int(iset/10)+48)
            tableN(j+2:j+2) = char(iset-10*int(iset/10)+48)
         endif
         tableN(j+3:j+6) = '.pds'
         write (*,*) tableN
         IU= NextUn()
         Open(IU, File=tableN, Status='OLD', Err=100)
         Call Readpds0Multi (IU, iset)
         Close (IU)
      enddo
      do ipts = 1, MXPQX
         upd(ipts) = updmulti(ipts)
      enddo
...
C               *************************
      ...
      Subroutine Readpds0Multi (Nu, iset)
      Implicit Double Precision (A-H,O-Z)
      Character Line*80
      integer ipdsformat
      PARAMETER (MXX = 201, MXQ = 40, MXF = 6, MaxVal=4)
...
c      Read  (Nu, *, IOSTAT=IRET) (UPD(I), I=1,Npts)
      Read  (Nu, *, IOSTAT=IRET) (UPDMULTI(I + MXPQX*iset), I=1,Npts)
...
cz Modified 6/18/2024 to access set through updmulti 
...
cz Get offset into updmulti
      ioffset = icallset*MXPQX
...
C                           (as we do for q), since Upd(J1) is undefined.
            Fij(1) = 0
            Fij(2) = UpdMulti(ioffset + J1+1) * XV(1)**2
            Fij(3) = UpdMulti(ioffset + J1+2) * XV(2)**2
            Fij(4) = UpdMulti(ioffset + J1+3) * XV(3)**2
c            Fij(2) = Upd(J1+1) * XV(1)**2
c            Fij(3) = Upd(J1+2) * XV(2)**2
c            Fij(4) = Upd(J1+3) * XV(3)**2

C
C                 Use Polint which allows x to be anywhere w.r.t. the grid
c$$$            print *, "hello world 1"
cz         call polint_wrapper (XVpow(0), Fij(1), ss, Fx)
...
cz        call polint_wrapper (XVpow(Nx-3), Upd(J1), ss, Fx)
        call polint4f (XVpow(Nx-3), UpdMulti(ioffset + J1), ss, Fx)
...
C                              This applied to (JLx.Ge.2 .and. JLx.Le.Nx-2)
         ...
         sf2 = UpdMulti(ioffset + J1+1)
         sf3 = UpdMulti(ioffset + J1+2)
c         sf2 = Upd(J1+1)
c         sf3 = Upd(J1+2)
...
c         Fvec(it) = (const5*(Upd(J1)-g1)
c     &               + const6*(Upd(J1+3)-g4)
         Fvec(it) = (const5*(UpdMulti(ioffset + J1)-g1)
     &               + const6*(UpdMulti(ioffset + J1+3)-g4)
     &               + sf2*sy3 - sf3*sy2) / s23
